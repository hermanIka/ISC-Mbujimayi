...
router.post("/chapters/:chapterId/materials", requireTeacher, async (req, res): Promise<void> => {
  ...
  const [material] = await db
    ...
    .values({ id: nanoid(), chapterId, type: matType, url, fileName, fileSize })
    .returning();
...
router.delete("/materials/:id", requireTeacher, async (req, res): Promise<void> => {
  ...
  if (material.url.startsWith("/api/storage/objects/")) {
    const objectPath = material.url.replace("/api/storage", "");
    try {
      const objectFile = await objectStorageService.getObjectEntityFile(objectPath);
      await objectFile.delete();
      logger.info({ id, objectPath }, "🗑️ [MATERIALS] Objet GCS supprimé");
    } catch (err) {
...
router.get("/materials/:id/download", requireAuth, async (req, res): Promise<void> => {
  ...
  if (material.url.startsWith("/api/storage/objects/")) {
    ...
    try {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="${encodeURIComponent(material.fileName ?? "fichier")}"`);
      res.status(response.status);
      response.headers.forEach((value, key) => { if (key !== "content-disposition") res.setHeader(key, value); });
      if (response.body) {
      ...
      if (err instanceof ObjectNotFoundError) {
        ...
        logger.error({ err, id }, "Erreur de streaming objet GCS");
        res.status(500).json({ error: "Erreur lors du téléchargement" });
      }
    }
  } else {
    res.status(410).json({ error: "Ce support utilise l'ancien stockage local (non disponible)" });
  }
});
